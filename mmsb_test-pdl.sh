# key=1, data_set=2, mmsb=3, min_step=4, step=5, no_wait=6, N(doc)=7, M0(vocab)=8, rank=$9, d=$10, shuffleList=$11, initStepMultiplierMultiIter=$12 lambda=13 stepFactor=$14 
...
P0=1
key="mmsb_test"$1
#lambda=$(echo "scale=10; 0.1" | bc -l)
lambda=${13}
mean=$(echo "scale=10; $d-1" | bc -l)
sparse=0
kl=0
#nnmf=${13}
nnmf=1
...
#params=" -D mapred.job.reduce.memory.mb=$mmem -D mapred.child.java.opts=$jvm -D dsgd.regularizerLambda=$lambda -D dsgd.initMean=$mean -D dsgd.nnmf=$nnmf -D dsgd.sparse=$sparse -D dsgd.KL=$kl -D dsgd.N=$N -D dsgd.M0=$M0 -D dsgd.P0=$P0 -D dsgd.rank=$rank -D dsgd.debug=$debug -D dsgd.mmsb=$mmsb -D dsgd.shuffleList=$shuffleList -D dsgd.initStepMultiplierMultiIter=$initStepMultiplierMultiIter -D dsgd.no_wait=$no_wait -D mapred.reduce.tasks=$d $d 3 1 $key $dataset_path"
